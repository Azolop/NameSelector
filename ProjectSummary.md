...
- 周次情况记录表：输入开学日期后一次性生成第 1~24 周的 JSON（每周一张，独立文件 `weekly\日期跨度_周次.json`，如 `2026-08-31_2026-09-06_week1.json`，不使用中文命名）；下拉框直接选 1~24 周。
- 周次表前两列为「组号 + 成员」，之后列头为背默（2 列）、课堂（5 列）、作业（3 列）、总结（1 列），每个交叉单元格为可编辑字符串。
- 第 1 周起点为开学日期所在周的周一（如 9 月 1 日周二开学，则从 8 月 31 日周一算起）；开学日期记录在 `namelist.json` 的 `SemesterStart`。
- 重新生成时若 1~24 周已有表会先确认，确认后覆盖全部周次表。
...
- 生成周次表：输入开学日期（默认选中本机日期所在周），按周一规则一次生成 1~24 周独立 JSON；已有表时确认后覆盖。
...
- 最近提交：周次改为开学日期所在周的周一为第 1 周起点并一次生成 1~24 周；表格前两列改为组号 + 成员；名单格式改为「名称,组号,是否组长」。
