...
- `Dialogs/`：`NoticeDialog`（通用提示 / 确认对话框）、`DialogService`（统一对话框入口）
- 窗口：`MainWindow`（主界面）、`EditListDialog`（修改名单）、`ResultWindow`（选人结果）

## 提交记录

- 最近提交：所有 MessageBox 统一替换为 `Dialogs` 下的 `NoticeDialog` 风格对话框，大字号适配教室白板。
